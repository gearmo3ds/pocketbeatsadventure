...
static C3D_TexCube kitten_cube;
...
void cubeSceneInit(void)
{
    ...
    if (!texLoaded) {
        if (!loadTextureFromFile(&kitten_tex, &kitten_cube, "romfs:/gfx/kitten.t3x"))
            svcBreak(USERBREAK_PANIC);
        // loadTextureFromFile(&kitten_tex, &kitten_cube, "romfs:/gfx/kitten.t3x");
        texLoaded = true;
    }
...
void cubeRenderScene(C3D_RenderTarget* target)
{
	// Initialize graphics
    // gfxInitDefault();
