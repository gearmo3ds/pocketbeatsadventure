...
static bool music_preload(u32 need_at_least) {
    ...
    if (need_at_least == 0) need_at_least = AUDIO_BLOCKSIZE * 16;
...
int audio_init_preload(void) {
    ...
    // Runs on Application core separately
    thread = threadCreate(audio_thread, NULL, THREAD_STACKSZ, prio, 1, false);
